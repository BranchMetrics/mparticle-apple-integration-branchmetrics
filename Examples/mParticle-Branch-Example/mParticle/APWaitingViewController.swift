...
class APWaitingViewController: UIViewController {
    
    private static var globalWaitingViewController: APWaitingViewController? = nil
    ...
    class func instanceFromNib() -> APWaitingViewController {
        let controller = UINib(nibName: "APWaitingViewController", bundle: nil).instantiate(
            withOwner: nil,
            options: nil)[0] as! APWaitingViewController
        return controller
    }
            ...
            animations: {
                self.parentView?.transform =
                    (self.parentView?.transform.scaledBy(
                        x: APWaitingViewController.kScale,
                        y: APWaitingViewController.kScale
                    ))!
                self.view.transform = CGAffineTransform(
                    scaleX: 1.0 / APWaitingViewController.kScale,
                    y: 1.0 / APWaitingViewController.kScale
                )
    ...
    static func showWithMessage(message: String,
        ...
        if (APWaitingViewController.globalWaitingViewController == nil) {
            APWaitingViewController.globalWaitingViewController = APWaitingViewController()
        }
    ...
    static func hideWithMessage(message: String) {
        ...
        afterSecondsPerformBlock(seconds: waitingViewDefaultHangTime) {
            UIView.animate(withDuration: 1.0,
                           ...
                           completion: { (finished) in APWaitingViewController.hide() })
        }
    }
